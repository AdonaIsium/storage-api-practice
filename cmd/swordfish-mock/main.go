...
func main() {
	...
	jobs.RegisterCreateHost(reg)
	jobs.RegisterMapVolume(reg)
	jobs.RegisterUnmapVolume(reg)
	...
	// HTTP
	conn := core.NewConnectivityService(core.ConnectivityDeps{
		Volumes: vols, Hosts: hosts, Mappings: maps, Jobs: jobsRepo, Idempotency: idem, Audit: audit, Runner: runner, Now: time.Now,
	})

	srv := swordfish.New(deps, prov, conn)
	mux := srv.Routes()
